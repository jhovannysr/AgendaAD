...
public class Agenda {
	...
	/**
	 * (Jhovanny) Posiciona el puntero en la última línea para ingresar nuevos
	 * registros después del último registro que contiene el fichero
	 * 
	 * @param contacto
	 * @return
	 * @throws IOException
	...
	/**
	 * (Jhovanny) Almacena en el fichero el nuevo regsitro
	 * 
	 * @param contacto
	 * @throws IOException
	 */
	private void almacenarContacto(Contacto contacto) throws IOException {

		String registro = String.format("%s,%s,%s,%d\n", contacto.getUuid().toString(), contacto.getNombre(),
				contacto.getTelefono(), contacto.getEdad());
		raf_file.writeBytes(registro);
	}

	/**
	 * (Cristian) Lista de todos los registros del fichero
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * (Jhovanny) Almacenar en un objeto un registro del fichero
	 * 
	 * @return
	 * @throws IOException
	 */
	private Contacto leerContacto() throws IOException {
		...
		var c = new Contacto(UUID.fromString(campos[0]), campos[1], campos[2], Integer.parseInt(campos[3]));
		return c;
	}

	/**
	 * (JeanPaul) Consulta de contacto por nombre
	 * 
	 * @param nombre
	 * @return
	 * @throws IOException
	...
	public Contacto consultaPorNombre(String nombre) throws IOException {
		...
		while (raf_file.getFilePointer() < raf_file.length()) {
			var c = leerContacto();
			if (c.getNombre().length() >= nombre.length()) {
				for (int i = 0; i < nombre.length(); i++) {
					if (nombre.charAt(i) == c.getNombre().charAt(i)) {
					} else {
						coincide = false;
					}
				}
	...
	/**
	 * (JeanPaul) Consulta de contacto por codigo
	 * 
	 * @param codigo
	 * @return
	 * @throws IOException
	...
	/**
	 * (Jhovanny) Almacenar los registros a un fichero temporal excepto el contacto
	 * eliminado
	 * 
	 * @return
	 * @throws IOException
	 */
	private String eliminarContactoSeleccionado(String codigoContacto) throws IOException {
	...
	/**
	 * (Jhovanny) Elimina el fichero principal y renombra el fichero temporal por el
	 * principal
	 * 
